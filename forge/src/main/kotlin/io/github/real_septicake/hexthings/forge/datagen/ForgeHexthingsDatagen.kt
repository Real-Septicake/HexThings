package io.github.real_septicake.hexthings.forge.datagen
...
object ForgeHexthingsDatagen {
    fun init(event: GatherDataEvent) {
        event.apply {
            // common datagen
            if (System.getProperty("hexthings.common-datagen") == "true") {
                // TODO: add datagen providers
            }

            // Forge-only datagen
            if (System.getProperty("hexthings.forge-datagen") == "true") {
                addVanillaProvider(includeServer()) { HexthingsActionTags(it, lookupProvider) }
            }
        }
    }
}
