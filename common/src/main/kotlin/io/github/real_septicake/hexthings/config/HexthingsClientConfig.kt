package io.github.real_septicake.hexthings.config
...
object HexthingsClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HexthingsServerConfig.holder.config = HexthingsServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HexthingsServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hexthings.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HexthingsServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HexthingsServerConfig.ServerConfig()
    }
